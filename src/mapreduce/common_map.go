...
func doMap(
	...
	interKV := mapF(inFile, string(content))
	...
	for reduceTaskNumber := 0; reduceTaskNumber < nReduce; reduceTaskNumber++ {
		tmpFileName := reduceName(jobName, mapTaskNumber, reduceTaskNumber)
		f, err := os.Create(tmpFileName)
		fileList[reduceTaskNumber] = f

		defer fileList[reduceTaskNumber].Close()

		if err != nil {
			panic(err)
		}

		encoderList[reduceTaskNumber] = json.NewEncoder(fileList[reduceTaskNumber])
	}
	...
	for _, kv := range interKV {
		...
		encoderList[reduceTaskNumber].Encode(kv)
	}
}
